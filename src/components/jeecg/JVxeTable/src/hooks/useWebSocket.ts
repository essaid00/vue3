...
const vs = {
  ...
  sendMessage(type, message) {
    try {
      ...
      console.warn('[JVxeWebSocket] Échec de l envoi du message :(' + err.code + ')');
    }
  },
  ...
  reconnect() {
    ...
    setTimeout(() => {
      ...
      this.ws = null;
      console.info('【JVxeWebSocket】Essayez de vous reconnecter...');
  ...
  on: {
    open() {
      console.info('【JVxeWebSocket】Connexion réussie');
      this.heartCheck.start();
    },
    error(e) {
      console.warn('[JVxeWebSocket] Une erreur de connexion s est produite:', e);
      this.reconnect();
    },
    message(e) {
      // 解析消息
      ...
      try {
        ...
        console.warn('[JVxeWebSocket] Reçu un message non analysable:', e.data);
        return;
      }
      ...
      switch (type) {
        ...
        case this.constants.TYPE_UVT:
          ...
          break;
        default:
          console.warn('[JVxeWebSocket] Type de message non reconnu reçu:' + type);
          break;
      }
    },
    close(e) {
      console.info('[JVxeWebSocket] La connexion a été fermée:', e);
      this.reconnect();
    },
  },
